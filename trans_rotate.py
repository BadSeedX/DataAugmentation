...
from PIL import ImageDraw #version 6.1.0
...
def rotate(angle):
    ...
    # 读取图片和目标框信息
    img_path = "./data/rawImages/228.jpg"
    img = Image.open(img_path)
    bbox_file_path = "./data/txt/228.txt"
    bbox_file = open(bbox_file_path)
    bbox_contents = bbox_file.readlines()

    # 将目标检测框坐标转化为数组
    bboxes = np.zeros((len(bbox_contents), 4), dtype=int)
    for i in range(len(bbox_contents)):
        for j in range(4):
            bboxes[i][j] = int(float(bbox_contents[i].split(' ')[j+1]))
    ...
    corners.reshape(len(bbox_contents), 8)
...
if __name__ == "__main__":
    rotate(240).save('test.jpg')
    """
    plt.figure("test")
    plt.imshow(rotate(30))
    plt.show()
    """